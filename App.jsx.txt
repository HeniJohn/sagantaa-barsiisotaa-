...
import step1Img from "./assets/tutorial/step1-teachers.png";
import step2Img from "./assets/tutorial/step2-assignments.png";
import step3Img from "./assets/tutorial/step3-schedule.png";
import step4Img from "./assets/tutorial/step4-edit-nav.png";
import step5Img from "./assets/tutorial/step5-teacher-login.png";
import {
  Plus,
  Trash2,
  Users,
...
const STRINGS = {
  om: {
    ...
    menuUsage: "Qajeelfama Fayyadamaa",
    usageStep1Title: "1. Barsiisota, Kutaalee fi Barnoota galchi",
    usageStep1Desc:
      "Tab \"Barsiisota\" jalatti maqaa barsiisaa (fkn. Obbo Guddataa Bayyanaa) galchii \"Add\" tuqi. \"Kutaalee\" jalatti maqaa kutaa (fkn. Kutaa 9A), \"Barnoota\" jalatti immoo maqaa barnootaa (fkn. Herrega) akkasuma galchi.",
    usageStep2Title: "2. Ramaddii uumi",
    usageStep2Desc:
      "Tab \"Ramaddii\" jalatti barsiisaa (Obbo Guddataa Bayyanaa), barnoota (Herrega), kutaa (Kutaa 9A) fi sa'aatii/torban filadhuutii \"Add\" tuqi. Tokkoon tokkoon barnootaa ramaddii addaa qaba.",
    usageStep3Title: "3. Sagantaa Uumi tuqi",
    usageStep3Desc: "Tab \"Sagantaa\" jalatti sa'aatii guyyaa tokkotti murteessitee \"Sagantaa Uumi\" tuqi. Sagantichi ofumaan walitti dhufeenya (wal-irra-buluu) hin qabu — barsiisaan tokko yeroo tokkotti kutaa lama hin barsiisu.",
    usageStep4Title: "4. Yoo barbaachise gulaali",
    usageStep4Desc: "\"Sagantaa\" jalatti \"Kutaan ilaali\" filadhuutii qabduu ✏️ \"Gulaali\" jedhu tuqi. Sa'aatii tokko tuqii sa'aatii biraa tuqi — wal isaanii jijjiiru (swap). Sa'aatii duwwaa argite yoo jiraate, sagantaa hin argatin (unplaced) filadhuutii bakka duwwaa keessatti kaa'i. Qabduu ✕ tiin sa'aatii tokko haqi.",
    usageStep5Title: "5. Barsiisota waliif qoodi",
    usageStep5Desc: "Barsiisonni (fkn. Obbo Guddataa Bayyanaa) gahee \"Barsiisaa\" filatanii maqaa isaanii filachuun sagantaa mataa isaanii qofa ilaaluu danda'u (gulaaluu hin danda'an). Suura (PNG) ykn PDF walitti qabuu dandeessa — menu (⋮) jalatti.",
    editSchedule: "Gulaali",
    doneEditing: "Xumuri",
    editHintClass: "Sa'aatii tokko tuqi, ergasii sa'aatii biraa tuqi — wal isaanii jijjiiru. Qabduu ✕ tiin haqi.",
    editHintTeacherView: "Gulaaluuf jecha \"Kutaan ilaali\" jedhu filadhu.",
    unplacedTitle: "Sa'aatii bakka hin argatin — tuqii bakka duwwaa keessatti kaa'i",
    teacherConflictMsg: "Hin danda'amu — barsiisichi yeroo kanatti kutaa biraa keessa jira.",
    cellOccupiedMsg: "Bakki kun duraan qabameera — jalqaba haqi.",
  },
  en: {
    ...
    menuUsage: "How to Use",
    usageStep1Title: "1. Add teachers, classes and subjects",
    usageStep1Desc:
      "Under \"Teachers\", add a name (e.g. Mr. Guddataa Bayyanaa) and tap \"Add\". Under \"Classes\" add a class (e.g. Grade 9A), and under \"Subjects\" add a subject (e.g. Mathematics) the same way.",
    usageStep2Title: "2. Create assignments",
    usageStep2Desc:
      "Under \"Assignments\", pick a teacher (Mr. Guddataa Bayyanaa), subject (Mathematics), class (Grade 9A) and periods/week, then tap \"Add\". Each subject gets its own assignment row.",
    usageStep3Title: "3. Tap Generate Schedule",
    usageStep3Desc: "Under \"Schedule\", set periods per day and tap \"Generate Schedule\". The result is automatically conflict-free — no teacher is ever double-booked into two classes at once.",
    usageStep4Title: "4. Fine-tune it if needed",
    usageStep4Desc: "Under \"Schedule\", switch to \"View by Class\" and tap the ✏️ \"Edit\" button. Tap one period, then another, to swap them. If a period couldn't be placed, tap it in the \"unplaced\" list, then tap an empty cell to drop it in. Use the ✕ on a cell to clear it.",
    usageStep5Title: "5. Share it with teachers",
    usageStep5Desc: "Teachers (e.g. Mr. Guddataa Bayyanaa) can switch to \"Teacher\" mode and pick their name to see only their own schedule (view-only). Export as PNG or PDF from the menu (⋮).",
    editSchedule: "Edit",
    doneEditing: "Done",
    editHintClass: "Tap one period, then another, to swap them. Use the ✕ to clear a period.",
    editHintTeacherView: "Switch to \"View by Class\" to edit.",
    unplacedTitle: "Periods that couldn't be placed — tap one, then tap an empty cell",
    teacherConflictMsg: "Can't do that — that teacher is already teaching another class at that time.",
    cellOccupiedMsg: "That period is already taken — clear it first.",
  },
};
...
function generateSchedule(data) {
  ...
  // Guard against assignments that reference a teacher/class/subject that has
  // since been deleted (this used to slip through and could corrupt the grid
  // or crash the generator — now those stale rows are simply skipped).
  const teacherIds = new Set(data.teachers.map((x) => x.id));
  const classIds = new Set(data.classes.map((x) => x.id));
  const subjectIds = new Set(data.subjects.map((x) => x.id));
  const validAssignments = data.assignments.filter(
    (a) =>
      teacherIds.has(a.teacherId) &&
      classIds.has(a.classId) &&
      subjectIds.has(a.subjectId) &&
      a.periodsPerWeek > 0
  );

  // Assignments that need the most periods per week are the hardest to fit in,
  // so they get first pick of slots. Placing them last (as before) meant they
  // were the ones most likely to run out of room and collide/overlap or get
  // dropped — placing them first fixes that.
  const sortedAssignments = [...validAssignments].sort((a, b) => b.periodsPerWeek - a.periodsPerWeek);

  const sessions = [];
  sortedAssignments.forEach((a) => {
    for (let i = 0; i < a.periodsPerWeek; i++) sessions.push(a);
  });
  ...
  // Tries to place one session. When avoidAdjacent is true it additionally
  // refuses a slot that sits right before/after another period of the exact
  // same subject in the same class — this is the "back-to-back" problem the
  // old version didn't guard against.
  const tryPlace = (s, avoidAdjacent) => {
    ...
    for (const di of dayOrder) {
      for (let p = 0; p < periodsPerDay; p++) {
        ...
        if (avoidAdjacent) {
          const prev = byClass[s.classId][di][p - 1];
          const next = byClass[s.classId][di][p + 1];
          if ((prev && prev.subjectId === s.subjectId) || (next && next.subjectId === s.subjectId)) continue;
        }
  ...
  // Pass 1: strict, no back-to-back repeats of the same subject.
  const leftover = [];
  for (const s of sessions) {
    if (!tryPlace(s, true)) leftover.push(s);
  }
  // Pass 2: relax that preference for whatever didn't fit, so only genuine
  // scheduling conflicts (not just a style preference) end up unplaced.
  const unplaced = [];
  for (const s of leftover) {
    if (!tryPlace(s, false)) unplaced.push(s);
  }
...
// Every helper below returns a *new* schedule object (never mutates the one
// passed in) so it can be handed straight to setData for saving, and every
// edit re-checks the same "is this teacher already busy at this exact
// day/period in another class" rule the generator itself uses — so hand
// edits can never introduce the overlap the generator was built to prevent.

function teacherBusySlots(schedule, excludeClassId) {
  const busy = {};
  Object.entries(schedule.byClass).forEach(([classId, days]) => {
    if (classId === excludeClassId) return;
    Object.entries(days).forEach(([di, periods]) => {
      Object.entries(periods).forEach(([p, cell]) => {
        if (!cell) return;
        const key = `${di}|${p}`;
        if (!busy[key]) busy[key] = new Set();
        busy[key].add(cell.teacherId);
      });
    });
  });
  return busy;
...
function swapScheduleCells(schedule, classId, a, b) {
  const grid = schedule.byClass[classId];
  const cellA = grid[a.di]?.[a.p] || null;
  const cellB = grid[b.di]?.[b.p] || null;
  if (!cellA && !cellB) return { schedule, error: null };
  if (a.di === b.di && a.p === b.p) return { schedule, error: null };

  const busy = teacherBusySlots(schedule, classId);
  const busyAt = (di, p, teacherId) => teacherId && busy[`${di}|${p}`]?.has(teacherId);

  if (busyAt(a.di, a.p, cellB?.teacherId)) return { schedule, error: "teacherConflict" };
  if (busyAt(b.di, b.p, cellA?.teacherId)) return { schedule, error: "teacherConflict" };

  const nextGrid = { ...grid };
  if (a.di === b.di) {
    nextGrid[a.di] = { ...grid[a.di], [a.p]: cellB, [b.p]: cellA };
  } else {
    nextGrid[a.di] = { ...grid[a.di], [a.p]: cellB };
    nextGrid[b.di] = { ...grid[b.di], [b.p]: cellA };
  }
  return { schedule: { ...schedule, byClass: { ...schedule.byClass, [classId]: nextGrid } }, error: null };
}

function clearScheduleCell(schedule, classId, di, p) {
  const grid = schedule.byClass[classId];
  const cell = grid[di]?.[p];
  if (!cell) return schedule;
  const nextGrid = { ...grid, [di]: { ...grid[di], [p]: null } };
  return {
    ...schedule,
    byClass: { ...schedule.byClass, [classId]: nextGrid },
    unplaced: [...schedule.unplaced, { id: cell.assignmentId }],
  };
}

function placeScheduleCell(schedule, classId, di, p, assignment) {
  const grid = schedule.byClass[classId];
  if (grid[di]?.[p]) return { schedule, error: "cellOccupied" };
  const busy = teacherBusySlots(schedule, classId);
  if (busy[`${di}|${p}`]?.has(assignment.teacherId)) return { schedule, error: "teacherConflict" };

  const nextGrid = {
    ...grid,
    [di]: {
      ...grid[di],
      [p]: { teacherId: assignment.teacherId, subjectId: assignment.subjectId, assignmentId: assignment.id },
    },
  };
  let removed = false;
  const nextUnplaced = schedule.unplaced.filter((u) => {
    if (!removed && u.id === assignment.id) {
      removed = true;
      return false;
    }
    return true;
  ...
  return {
    schedule: { ...schedule, byClass: { ...schedule.byClass, [classId]: nextGrid }, unplaced: nextUnplaced },
    error: null,
  };
}
      ...
      >
        ...
        {Array.from({ length: periodsPerDay }).map((_, p) => (
          <React.Fragment key={p}>
            ...
            {dayLabels.map((_, di) => {
              const cell = cellFor(di, p);
              const selected = editMode && isSelected && isSelected(di, p);
              const armed = editMode && !cell && isArmedTarget && isArmedTarget(di, p);
              return (
                <div
                  key={di}
                  onClick={editMode && onCellClick ? () => onCellClick(di, p) : undefined}
                  className={`relative border-b border-r border-border last:border-r-0 p-1.5 min-h-58 ${
                    editMode ? "cursor-pointer" : ""
                  } ${selected ? "ring-2 ring-inset ring-accent" : ""} ${
                    armed ? "ring-2 ring-inset ring-teal-400" : ""
                  }`}
                >
                  {cell ? (
                    <div
                    ...
                    >
                      ...
                      {editMode && onClearCell && (
                        <button
                          ...
                          style={{ background: "rgba(0,0,0,0.25)", color: cell.color.text }}
                          aria-label="clear"
                        >
                          <X size={10} />
                        </button>
...
function ScheduleTab({ data, setData, t, registerExport }) {
  ...
  const [selectedCell, setSelectedCell] = useState(null); // {di, p} first tap of a swap
  const [armedUnplacedId, setArmedUnplacedId] = useState(null); // assignment id chosen for placement
  const [editMsg, setEditMsg] = useState(""); // transient feedback (conflict, etc.)
  ...
  useEffect(() => {
    ...
    setArmedUnplacedId(null);
    setEditMsg("");
  }, [viewMode, selectedId, editMode]);
  ...
  const flashMsg = (msg) => {
    setEditMsg(msg);
    setTimeout(() => setEditMsg((m) => (m === msg ? "" : m)), 2200);
  };

  const canEditHere = editMode && viewMode === "class" && !!schedule && !!selectedId;

  const handleCellClick = (di, p) => {
    if (!canEditHere) return;
    if (armedUnplacedId) {
      const assignment = assignments.find((a) => a.id === armedUnplacedId);
      if (!assignment) {
        setArmedUnplacedId(null);
        return;
      }
      const { schedule: next, error } = placeScheduleCell(schedule, selectedId, di, p, assignment);
      if (error) {
        flashMsg(error === "teacherConflict" ? t.teacherConflictMsg : t.cellOccupiedMsg);
        return;
      }
      setArmedUnplacedId(null);
    ...
    if (!selectedCell) {
      setSelectedCell({ di, p });
      return;
    }
    if (selectedCell.di === di && selectedCell.p === p) {
      setSelectedCell(null);
      return;
    }
    const { schedule: next, error } = swapScheduleCells(schedule, selectedId, selectedCell, { di, p });
    setSelectedCell(null);
    if (error) {
      flashMsg(t.teacherConflictMsg);
      return;
    }
    setData({ ...data, schedule: next });
  };

  const handleClearCell = (di, p) => {
    if (!canEditHere) return;
    const next = clearScheduleCell(schedule, selectedId, di, p);
    setSelectedCell(null);
    setData({ ...data, schedule: next });
  };

  const classUnplaced =
    schedule && viewMode === "class" && selectedId
      ? schedule.unplaced
          .map((u) => assignments.find((a) => a.id === u.id))
          .filter((a) => a && a.classId === selectedId)
      : [];
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap items-end gap-3 mb-5">
        ...
        {schedule && (
          <Btn
            ...
            disabled={viewMode !== "class" || !selectedId}
          >
            {editMode ? <Check size={16} /> : <Pencil size={16} />}
            {editMode ? t.doneEditing : t.editSchedule}
          </Btn>
        )}
      ...
      {editMode && (
        <div className="flex items-start gap-2 bg-panel-soft border border-border-strong rounded-xl px-3.5 py-3 mb-5 text-xs text-text-dim">
          <Info size={14} className="text-accent shrink-0 mt-0.5" />
          <span>{viewMode === "class" ? t.editHintClass : t.editHintTeacherView}</span>
        </div>
      )}

      {editMsg && (
        <div className="flex items-center gap-2 bg-danger-10 border border-danger-30 rounded-xl px-3.5 py-2.5 mb-4 text-xs text-danger">
          <AlertTriangle size={13} className="shrink-0" />
          {editMsg}
        </div>
      )}

      {editMode && viewMode === "class" && classUnplaced.length > 0 && (
        <div className="mb-4">
          <p className="text-text-faint text-11 uppercase tracking-wide mb-1.5">{t.unplacedTitle}</p>
          <div className="flex flex-wrap gap-1.5">
            {classUnplaced.map((a, i) => {
              const armed = armedUnplacedId === a.id;
              const color = teacherColor(teachers, a.teacherId);
              return (
                <button
                  key={`${a.id}-${i}`}
                  onClick={() => setArmedUnplacedId(armed ? null : a.id)}
                  className={`px-2.5 py-1.5 rounded-lg text-11 font-semibold border-2 transition-colors ${
                    armed ? "border-teal-400" : "border-transparent"
                  }`}
                  style={{ background: color.bg, color: color.text }}
                >
                  {nameOf(subjects, a.subjectId)} · {nameOf(teachers, a.teacherId)}
                </button>
              );
            })}
          </div>
      ...
      {!schedule ? (
        ...
        <ScheduleGrid
          ...
          isSelected={(di, p) => selectedCell && selectedCell.di === di && selectedCell.p === p}
          isArmedTarget={() => !!armedUnplacedId}
...
function TopMenu({ theme, setTheme, lang, setLang, t, onSavePdf, onSaveImage, canExport, onOpenUsage }) {
          ...
          {onOpenUsage && (
            <div className="px-4 py-3 border-t border-border">
              <button
                ...
                className="w-full flex items-center gap-2 text-sm text-text hover-text-accent transition-colors"
              >
...
function UsageGuideModal({ t, onClose }) {
  const steps = [
    { icon: Users, title: t.usageStep1Title, desc: t.usageStep1Desc, img: step1Img },
    { icon: ClipboardList, title: t.usageStep2Title, desc: t.usageStep2Desc, img: step2Img },
    { icon: CalendarDays, title: t.usageStep3Title, desc: t.usageStep3Desc, img: step3Img },
    { icon: Pencil, title: t.usageStep4Title, desc: t.usageStep4Desc, img: step4Img },
    { icon: GraduationCap, title: t.usageStep5Title, desc: t.usageStep5Desc, img: step5Img },
  ];
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
      <div className="absolute inset-0" style={{ background: "rgba(0,0,0,0.55)" }} onClick={onClose} />
      <div className="relative w-full max-w-lg max-h-[85vh] overflow-y-auto bg-panel border border-border-strong rounded-2xl shadow-2xl">
        <div className="flex items-center justify-between px-5 py-4 border-b border-border sticky top-0 bg-panel">
          <span className="font-display text-lg text-text">{t.usageTitle}</span>
          <button onClick={onClose} className="text-text-faint hover-text-text" aria-label={t.menuClose}>
            <X size={16} />
          </button>
        </div>
              ...
              <div key={i}>
                ...
                {s.img && (
                  <img
                    src={s.img}
                    alt={s.title}
                    className="w-full rounded-xl border border-border-strong shadow-lg"
                  />
                )}
              </div>
            );
